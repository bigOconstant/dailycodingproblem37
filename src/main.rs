...
/*
Convert to binary,iterate to 2^n if 1 add if 0 don't add is added example
*/
...
fn power_set(input:Vec<i32>)-> Vec<Vec<i32>>{
    ...
    for x in 0..length  {
        let mut bin:String = format!("{:b}",x);
        let mut bin = pad_zeros(bin,(input.len() -1) as i32);
...
fn pad_zeros(mut input:String,pad_size:i32)->String{
